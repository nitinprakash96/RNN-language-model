...
class CharRNN(object):
    ...
    def __init__(self,
        ...
        self.targets = tf.placeholder(
            tf.int64, [self.batch_size, self.num_unrollings], name='targets')
        ...
        cell = cell_fn(
            self.hidden_size, reuse=tf.get_variable_scope().reuse, **params)
        ...
        with tf.name_scope('initial_state'):
            ...
            self.zero_state = multi_cell.zero_state(self.batch_size,
                                                    tf.float32)
        ...
        with tf.name_scope('embedding_layer'):
            if embedding_size > 0:
                ...
                self.embedding = tf.constant(
                    np.eye(self.vocab_size), dtype=tf.float32)
        ...
        outputs, final_state = tf.contrib.rnn.static_rnn(
            multi_cell, sliced_inputs, initial_state=self.initial_state)

        self.final_state = final_state

        with tf.name_scope('flatten_ouputs'):
            # Flatten the outputs into one dimension.
            flat_outputs = tf.reshape(
                tf.concat(axis=1, values=outputs), [-1, hidden_size])

        with tf.name_scope('flatten_targets'):
            # Flatten the targets too.
            flat_targets = tf.reshape(
                tf.concat(axis=1, values=self.targets), [-1])
        ...
        with tf.name_scope('loss_monitor'):
            ...
            self.reset_loss_monitor = tf.group(
                sum_mean_loss.assign(0.0),
                count.assign(0.0),
        ...
        self.summaries = tf.summary.merge(
            [average_loss_summary, ppl_summary], name='loss_monitor')
    ...
    def run_epoch(self,
        ...
        for step in range(epoch_size // divide_by_n):
            ...
            ops = [
                self.average_loss, self.final_state, extra_op, self.summaries,
            ...
            if (verbose > 0) and ((step + 1) % freq == 0):
                logging.info(
                    "%.1f%%, step:%d, perplexity: %.3f, speed: %.0f words",
                    (step + 1) * 1.0 / epoch_size * 100, step, ppl,
                    (step + 1) * self.batch_size * self.num_unrollings /
                    (time.time() - start_time))

        logging.info("Perplexity: %.3f, speed: %.0f words per sec", ppl,
                     (step + 1) * self.batch_size * self.num_unrollings /
                     (time.time() - start_time))
    ...
    def sample_seq(self,
        ...
        for i in range(length):
            ...
            unnormalized_probs = np.exp(
                (logits - np.max(logits)) / temperature)
...
def create_tuple_placeholders_with_default(inputs, extra_dims, shape):
    if isinstance(shape, int):
        result = tf.placeholder_with_default(inputs,
                                             list(extra_dims) + [shape])
    else:
        subplaceholders = [
            create_tuple_placeholders_with_default(subinputs, extra_dims,
                                                   subshape)
