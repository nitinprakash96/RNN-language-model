...
def main():
    ...
    parser.add_argument(
        '--batch_size', type=int, default=20, help='minibatch size')
    parser.add_argument(
        '--train_frac',
        type=float,
        default=0.9,
    ...
    parser.add_argument(
        '--decay_rate', type=float, default=0.95, help='decay rate')
    ...
    parser.add_argument(
        '--init_model', type=str, default='', help=('initial model'))
    parser.add_argument(
        '--best_model', type=str, default='', help=('current best model'))
    parser.add_argument(
        '--best_valid_ppl',
        type=float,
        default=np.Inf,
    ...
    with graph.as_default():
        ...
        with tf.name_scope('evaluation'):
            ...
            saver = tf.train.Saver(
                name='checkpoint_saver', max_to_keep=args.max_to_keep)
    ...
    try:
        ...
        with tf.Session(graph=graph) as session:
            ...
            train_writer = tf.summary.FileWriter(args.tb_log_dir + 'train/',
                                                 graph_info)
            valid_writer = tf.summary.FileWriter(args.tb_log_dir + 'valid/',
                                                 graph_info)
            ...
            test_ppl, _, _ = test_model.run_epoch(
                session,
                test_size,
                test_batches,
                is_training=False,
